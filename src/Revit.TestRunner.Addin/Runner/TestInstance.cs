...
namespace Revit.TestRunner.Runner
{
    ...
    public class TestInstance
    {
        ...
        public TestState AnalyzeTestMethod( string methodName, bool isSingle )
        {
            ...
            var isExplicitMarked = MarkedByAttribute( method, "NUnit.Framework.ExplicitAttribute" ) ||
                                   IsExplicitMarked && !isSingle;
            var isIgnoreMarked = MarkedByAttribute( method, "NUnit.Framework.IgnoreAttribute" );
            var hasTestCaseAttribute = MarkedByAttribute( method, "NUnit.Framework.TestCaseAttribute" );
        ...
        public async Task ExecuteTestMethod( string methodName )
        {
            ...
            try {
                ...
                if( e != null && e.InnerException.GetType().Name.Contains( "SuccessException" ) ) {
                    // ignore, test pass legally by Assert.Pass()
                    Console.WriteLine( $"Pass: {e.Message}" );
                }
                else {
                    throw;
                }
        ...
        {
            mOneTimeSetUp = GetMethodByAttribute( Type, "NUnit.Framework.OneTimeSetUpAttribute" );
            mOneTimeTearDown = GetMethodByAttribute( Type, "NUnit.Framework.OneTimeTearDownAttribute" );
            mSetUp = GetMethodByAttribute( Type, "NUnit.Framework.SetUpAttribute" );
            mTearDown = GetMethodByAttribute( Type, "NUnit.Framework.TearDownAttribute" );

            IsExplicitMarked = MarkedByAttribute( Type.GetTypeInfo(), "NUnit.Framework.ExplicitAttribute" );
        }
        ...
        /// </summary>
        private MethodInfo GetMethodByAttribute( Type type, string attributeType )
        {
            ...
            if( listOfMethods.Count > 1 ) throw new InvalidOperationException( $"More than one method marked with '{attributeType}' attribute found!" );

            return listOfMethods.SingleOrDefault();
        }

        private bool MarkedByAttribute( MethodInfo methodInfo, string attributeType )
        {
            return methodInfo.GetCustomAttributes( true ).Select( a => a.ToString() ).Contains( attributeType );
        }

        private bool MarkedByAttribute( TypeInfo methodInfo, string attributeType )
        {
            return methodInfo.GetCustomAttributes( true ).Select( a => a.ToString() ).Contains( attributeType );
        }
